...
NucleusIQ is the practical way to build AI agents that work in real environments. Connect to OpenAI, Google Gemini, Groq, or MockLLM for testing — with the same agent code. NucleusIQ provides three execution modes, built-in file tools, the `@tool` decorator, memory strategies, plugins, streaming, structured output, usage tracking, cost estimation, and **context window management**.
...
-   :material-swap-horizontal: **Provider-portable**
    ---
    Swap OpenAI, Gemini, Groq, or Mock LLMs with one line. Same agent, same tools, same plugins.
